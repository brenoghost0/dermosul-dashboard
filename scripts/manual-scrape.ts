...
function extractListingProducts(jsonLdBlocks: string[], catalogUrl: string): ProductPreview[] {
  const base = new URL(catalogUrl);
  const results: ProductPreview[] = [];
  ...
  const handleProductNode = (node: any) => {
  ...
  for (const raw of jsonLdBlocks) {
    ...
    traverseJsonLd(parsed, (node) => {
      ...
      if (typeList.includes('ItemList') && Array.isArray((node as any).itemListElement)) {
        for (const entry of (node as any).itemListElement) {
          const item = entry?.item ?? entry;
          if (!item) continue;
          traverseJsonLd(item, (inner) => {
            if (!inner || typeof inner !== 'object') return;
            const innerType = (inner as any)['@type'] ?? (inner as any).type;
...
function mapNextProductCandidate(item: any, base: URL): ProductPreview | null {
...
function extractProductsFromNextData(nextDataRaw: string | null, catalogUrl: string): ProductPreview[] {
    ...
    if (Array.isArray(node)) {
      if (node.length && typeof node[0] === 'object') {
        const candidates = node
          ...
          .filter((maybe): maybe is ProductPreview => Boolean(maybe && maybe.url));
...
async function extractProductsFromDom(page: import('playwright').Page, catalogUrl: string): Promise<ProductPreview[]> {
  const raw = await page.evaluate(() => {
    ...
    const collect = (root: Element) => {
      const anchor = root instanceof HTMLAnchorElement ? root : (root.querySelector('a[href]') as HTMLAnchorElement | null);
      if (!anchor) return;
      const titleSource =
        root.querySelector('[data-testid="product-name"], [data-product-title], .product-name')?.textContent ||
        anchor.getAttribute('title') ||
        anchor.querySelector('[data-testid="product-name"]')?.textContent ||
        null;
      const brandSource =
        root.querySelector('[data-testid="product-brand"], .product-brand, .brand-name')?.textContent ||
        root.getAttribute('data-brand') ||
        anchor.dataset.brand ||
        anchor.getAttribute('data-brand') ||
        null;
      const priceSource =
        root.getAttribute('data-price') ||
...
function extractProductDetailFromJsonLd(jsonLdBlocks: string[], detailUrl: string): ProductDetail {
  ...
  const rawImages = (bestMatch as any).image;
  ...
  return {
    title: normalizeWhitespace((bestMatch as any).name ?? null),
    brand:
      typeof (bestMatch as any).brand === 'string'
        ? (bestMatch as any).brand
        : (bestMatch as any).brand?.name ?? null,
    price: parsePrice((bestMatch as any).offers?.price ?? (bestMatch as any).price),
    sku: (bestMatch as any).sku ?? (bestMatch as any).mpn ?? null,
    shortDescription: normalizeWhitespace((bestMatch as any).description ?? null),
    longDescriptionHtml: (bestMatch as any).description ?? null,
    images,
  };
}

function mergeImages(base: URL, ...imageLists: (string[] | undefined)[]): string[] | undefined {
...
async function main() {
  ...
  const sanitizeGalleryUrl = (raw: string | null | undefined, skuFragment?: string | null): string | null => {
    if (!raw) return null;
    let cleaned = raw
      .replace(/&(?:amp;)?quot;/gi, '')
      .replace(/&#x2F;/gi, '/')
  ...
  const collectGalleryUrlsFromPage = async (page: import('playwright').Page, skuFragment?: string | null) => {
  ...
  try {
    ...
    for (let i = 0; i < 40; i += 1) {
      ...
      for (const selector of loadMoreSelectors) {
        ...
        const isVisible = await button
          ...
          .catch(() => false);
        if (disabled !== null || !isVisible) continue;
    ...
    for (const productPreview of targets) {
      ...
      await detailPage.evaluate(() => {
        const clickIfVisible = (element: Element | null | undefined) => {
        ...
        for (const button of accordionButtons) {
          ...
          const isDetailControl =
            controls.includes('product-description') ||
      ...
      const domData = await detailPage.evaluate(() => {
        const pickText = (selector: string) => document.querySelector(selector)?.textContent?.trim() ?? null;
        ...
        const syndigoHtml = (window as any).blz?.product?.description ?? null;

        const normalizeParagraphs = (html: string | null | undefined) => {
        ...
        const bestParagraphs =
          detailParagraphs.length > 0 ? detailParagraphs : syndigoParagraphs;
        ...
        return {
          ...
          metaPrice,
          shortDescription:
            bestParagraphs.length > 0 ? bestParagraphs[0] : pickText('.short-description, .product-short-description') ||
    ...
    if (galleryImages.length === 0) {
      ...
      try {
        ...
        console.warn('[warn] Falha ao coletar imagens no fallback:', (error as Error).message);
      } finally {
      ...
      const merged: ProductDetail & { detailUrl: string } = {
