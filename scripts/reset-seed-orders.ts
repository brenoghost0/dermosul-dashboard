...
const id8 = (): string => Array.from({ length: 8 }, () => Math.floor(Math.random() * 10)).join('');
// CPF válido (com dígitos verificadores) e formatado XXX.XXX.XXX-XX
const generateValidCpf = (formatted = true): string => {
...
const generateBrMobile = (formatted = true): string => {
...
async function run() {
  ...
  for (let i = 0; i < 50; i++) {
    const lp = faker.helpers.arrayElement(landingPages);
    ...
    const customer = await prisma.customer.create({
      data: {
        ...
        gender: faker.helpers.arrayElement(['FEMININO','MASCULINO']),
        addresses: {
          create: {
            cep: realCep.cep,
    ...
    const status = faker.helpers.arrayElement(['pago', 'pendente', 'enviado', 'cancelado'] as const);
    const paymentMethod = faker.helpers.arrayElement(['cartao', 'pix', 'boleto'] as const) as PaymentMethod;
    ...
    await prisma.order.create({
      data: {
        ...
        status,
        category: faker.helpers.arrayElement(['Skincare', 'Maquiagem', 'Cabelo', 'Corpo']),
        totalAmount,
        createdAt,
        items: {
          create: [
