...
async function main() {
  ...
  const details = await page.evaluate(() => {
    const blzDesc = (window as any).blz?.product?.description ?? null;
    if (!blzDesc) return { blzDesc: null };
    const container = document.createElement('div');
    container.innerHTML = blzDesc;
    const paragraphs = Array.from(container.querySelectorAll('p')).map((p) => p.textContent?.trim())
      .filter((text): text is string => Boolean(text));
    return { blzDesc: blzDesc.slice(0, 200), paragraphs };
  });
