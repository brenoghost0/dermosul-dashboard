...
async function main() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    const landingPage = faker.helpers.arrayElement(landingPages);
    const qty = faker.number.int({ min: 1, max: 3 });
    const totalAmount = landingPage.price * qty;

    // Garante que os 10 primeiros pedidos sejam dos últimos 14 dias
    const createdAt = i < 10 ? faker.date.recent({ days: 14 }) : faker.date.past({ years: 1 });

    await prisma.order.create({
      data: {
        id: faker.string.alphanumeric(8).toUpperCase(),
        customerId: customer.id,
        status: faker.helpers.arrayElement(['pago', 'pendente', 'enviado', 'cancelado']),
        ...
        payments: {
          create: {
            paymentMethod: faker.helpers.arrayElement(['cartao', 'pix', 'boleto']),
