...
exports.cancelOrderWithRefund = cancelOrderWithRefund;
...
async function resolveProductForOrderItem(tx, rawItem) {
    ...
    if (!product) {
        const id = productId || (0, index_js_1.generateUniqueId)();
...
async function buildOrderItemCreate(tx, rawItem) {
...
function mapOrderFromPrisma(o) {
    ...
    return {
        ...
        fullId: o.id,
        createdAt: (0, index_js_1.toISO)(o.createdAt),
        ...
        timeline: [],
        paymentGateway: o.metadata && typeof o.metadata === 'object' && o.metadata?.paymentGateway
            ? o.metadata.paymentGateway
            : null,
    };
}
// --- Funções do Adaptador de Dados SQL ---
async function listOrders(params) {
    const { status, category, dateFrom, dateTo, sort, q, page = "1", pageSize = "20" } = params;
    const where = {};
    if (status) {
        where.status = status;
    }
...
async function createOrder(orderData) {
    ...
    const newOrder = await prisma_js_1.prisma.$transaction(async (tx) => {
        ...
        if (!customer) {
            customer = await tx.customer.create({
                data: {
                    ...
                    birthDate: orderData.customer.birthDate || (0, index_js_1.toISO)(new Date()),
        ...
        const itemsData = Array.isArray(orderData.items)
            ? await Promise.all(orderData.items.map((item) => buildOrderItemCreate(tx, item)))
            : [];
...
async function updateOrder(id, updatedFields) {
    const existingOrder = await prisma_js_1.prisma.order.findUnique({
        where: { id },
    ...
    if (updatedFields.customer || updatedFields.shipping) {
        let customer = await prisma_js_1.prisma.customer.findUnique({ where: { id: existingOrder.customerId } });
        if (customer) {
            customer = await prisma_js_1.prisma.customer.update({
            ...
            if (updatedFields.shipping?.postalCode) {
                let address = await prisma_js_1.prisma.address.findFirst({
                    where: {
                        customerId: customer.id,
                ...
                if (address) {
                    await prisma_js_1.prisma.address.update({
                        where: { id: address.id },
                        data: {
                    ...
                    await prisma_js_1.prisma.address.create({
                        data: {
                            customerId: customer.id,
                            cep: updatedFields.shipping.postalCode,
                            street: updatedFields.shipping.address1,
    ...
    if (updatedFields.status === 'cancelado') {
        await prisma_js_1.prisma.payment.updateMany({
            where: { orderId: id },
    ...
    const finalOrder = await prisma_js_1.prisma.order.findUnique({
        where: { id },
        include: {
            customer: {
                include: {
    ...
    try {
        ...
        if (changed && to) {
            if (nextStatus === 'pago') {
                ...
                await (0, mailer_js_1.sendMail)(to, `Dermosul • Pedido #${orderId} cancelado`, (0, mailer_js_1.renderCanceledEmail)({ name, orderId }));
            }
        }
    }
    catch (e) {
...
async function cancelOrderWithRefund(orderId, options = {}) {
    const order = await prisma_js_1.prisma.order.findUnique({
        where: { id: orderId },
        include: {
            customer: { include: { addresses: true } },
    ...
    const refundAmountCents = Math.max(0, Math.min(typeof options.refundAmountCents === "number" ? Math.round(options.refundAmountCents) : order.totalAmount, order.totalAmount));
    const metadataObject = order.metadata && typeof order.metadata === 'object' && order.metadata !== null
        ? { ...order.metadata }
        : {};
    const paymentGateway = metadataObject.paymentGateway || null;
    const gatewayPaymentId = paymentGateway?.gatewayPaymentId || null;
    let refundInfo = null;
    if (gatewayPaymentId) {
        const provider = (0, index_js_2.getPaymentProvider)();
    ...
    const updated = await prisma_js_1.prisma.order.findUnique({
        where: { id: orderId },
        include: {
            customer: { include: { addresses: true } },
            items: { include: { product: true } },
    ...
    try {
        ...
        console.warn('[email] falha ao avisar cancelamento:', e?.message || e);
    }
    return {
...
async function deleteOrder(id) {
    try {
        ...
        await prisma_js_1.prisma.order.delete({
            where: { id },
        });
        return true;
    }
...
async function paymentsBreakdown() {
    ...
    const recentPayments = await prisma_js_1.prisma.payment.findMany({
        where: {
            createdAt: {
                gte: fourteenDaysAgo,
            },
    ...
    for (const payment of recentPayments) {
        ...
        const amountInCents = payment.status === 'confirmado'
            ? paid
            : paid > 0
                ? paid
                : orderTotal;
...
async function listLandingPages() {
    try {
        const landingPages = await prisma_js_1.prisma.landingPage.findMany({
            orderBy: {
                createdAt: 'desc',
            },
            select: {
...
async function updateLandingPage(id, data) {
    const existingLandingPage = await prisma_js_1.prisma.landingPage.findUnique({
        where: { id },
    });
...
async function deleteLandingPage(id) {
    try {
        await prisma_js_1.prisma.landingPage.delete({
            where: { id },
        });
        return true;
    }
...
async function getLandingBySlug(slug) {
    const lp = await prisma_js_1.prisma.landingPage.findUnique({
        where: { slug },
        select: {
            id: true,
            slug: true,
...
async function createPublicOrder(orderData) {
    ...
    if (!address)
        errors.address = "Rua é obrigatória.";
    if (!addressNumber)
    ...
    if (!productTitle)
        errors.productTitle = "Título do produto é obrigatório.";
    const numQty = parseInt(qty, 10);
    if (isNaN(numQty) || numQty <= 0)
        errors.qty = "Quantidade deve ser um número positivo.";
    const numPrice = parseFloat(productPrice);
    if (isNaN(numPrice) || numPrice <= 0)
        errors.productPrice = "Preço do produto deve ser um número positivo.";
